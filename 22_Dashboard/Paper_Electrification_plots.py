...
def main():
    ...
    fig_5 = generate_electric_flight_operations_plots(Routes_and_Temp,Commercial_Batteries,aircraft,battery_choice,weight_fraction,system_voltage,propulsive_efficiency,percent_adoption,month_no,cost_of_electricity,switch_off)
...
def generate_electric_flight_operations_plots(Routes_and_Temp,Commercial_Batteries,aircraft,battery_choice,weight_fraction,system_voltage,propulsive_efficiency,percent_adoption,month_no,cost_of_electricity ,switch_off): 
    ...
    W_residual      = np.zeros(len(wf))
    passenger_reductions = np.zeros(len(wf))
    remaining_Pax   = np.zeros(len(wf))
    ...
    #Routes_and_Temp_Mo[i]['E_Passengers']   = remaining_Pax[i]   
