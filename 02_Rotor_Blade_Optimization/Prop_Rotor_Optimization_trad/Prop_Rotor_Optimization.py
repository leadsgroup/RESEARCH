...
def main():
    ...
    for i in range(len(alphas)): 
        for j in range(len(betas)):   
            ...
            problem.rotor_settings.beta  = jnp.array(betas[j])
            problem.rotor_settings.gamma = jnp.array(1.)
            problem.write_file = 'optimization_outputs' + str(alphas[i])+str(betas[j]) + '.txt'     
            ...
            # Output a file
            write_vsp_BEM_out(rotor, problem.results.hover.full_results,filename = 'BEMT_out' + str(alphas[i]) + str(betas[j]) +'.bem') 
            ...
            beta_opt_weight  = str(format(betas[j],'.5f'))
