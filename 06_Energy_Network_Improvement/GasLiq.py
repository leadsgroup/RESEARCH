...
from TMS.HSS.Kc_Ke_Factors.Kc_Ke_Lib import Kc_Ke_Lib, Kc_Ke_Lib_single_value, Kc_Ke_Lib_double_values
import pickle
...
class GasLiq_RectChan_OffDesign_Integrated:
    """ Design procedure in the compact heat exchanger:
        Input parameters: m_dot_h, C_R, Q_h, T_h_2

        Are we going to use the air velocity for the compact heat exchanger???
    """

    def Surface_Geom_Properties(self, b_c, b_h, beta_h, beta_c, d_H_h, d_H_c, L_c, L_h, H_stack, t_w):

        Surface_Geom_Properties = HEX_data_OffDesign()
        ...
        Surface_Geom_Properties.N_p             = N_p
        Surface_Geom_Properties.A_fr_h          = A_fr_h
        Surface_Geom_Properties.A_fr_c          = A_fr_c
        Surface_Geom_Properties.A_h             = A_h
        Surface_Geom_Properties.A_c             = A_c
        Surface_Geom_Properties.A_o_h           = A_o_h
        Surface_Geom_Properties.A_o_c           = A_o_c
        Surface_Geom_Properties.sigma_h         = sigma_h
        Surface_Geom_Properties.sigma_c         = sigma_c

        return Surface_Geom_Properties

    def Thermo_OffDesign(self, atmospheric_conditions, m_dot_h=None, m_dot_c=None, C_R=None, eff_HEX=None, T_h_1=None, T_c_1=None,
        ...
        ****************************************************************************"""
        ...
        Thermo_OffDesign = HEX_data_OffDesign()
        """*************************************************************************"""
        """output parameters"""
        ...
        # ---------------------------------------------------------------------------------
        air_velocity                    = atmospheric_conditions.velocity
        ambient_temperature             = atmospheric_conditions.temperature
        ambient_pressure                = atmospheric_conditions.pressure
        air_density                     = atmospheric_conditions.density
        sound_of_speed                  = atmospheric_conditions.speed_of_sound
        air_dynamic_viscosity           = atmospheric_conditions.dynamic_viscosity
        air_kinematic_viscosity         = atmospheric_conditions.kinematic_viscosity
        ...
        rho_c_2                         = CoolProp.PropsSI('D', 'P', p_c_2, 'T', T_c_2, 'Air')
        k_c_2                           = CoolProp.PropsSI('L', 'P', p_c_2, 'T', T_c_2, 'Air')
        cp_c_2                          = CoolProp.PropsSI('C', 'P', p_c_2, 'T', T_c_2, 'Air')
        mu_c_2                          = CoolProp.PropsSI('V', 'P', p_c_2, 'T', T_c_2, 'Air')
        Pr_c_2                          = CoolProp.PropsSI('Prandtl', 'P', p_c_2, 'T', T_c_2, 'Air')
        ...
        Thermo_OffDesign.Q              = Q
        Thermo_OffDesign.T_h_2          = T_h_2
        Thermo_OffDesign.T_c_2          = T_c_2
        Thermo_OffDesign.C_R            = C_R
        Thermo_OffDesign.NTU            = NTU
        Thermo_OffDesign.eff_HEX        = eff_HEX
        # new added
        Thermo_OffDesign.T_h_1          = T_h_1
        Thermo_OffDesign.T_c_1          = T_c_1
        # other results
        Thermo_OffDesign.Re_h           = Re_h
        Thermo_OffDesign.Re_c           = Re_c
        Thermo_OffDesign.eta_o_h        = eta_o_h
        Thermo_OffDesign.eta_o_c        = eta_o_c
        Thermo_OffDesign.h_h            = h_h
        Thermo_OffDesign.h_c            = h_c
        Thermo_OffDesign.f_h            = f_h
        Thermo_OffDesign.f_c            = f_c
        Thermo_OffDesign.G_h            = G_h
        Thermo_OffDesign.G_c            = G_c
        # thermophysical properties
        Thermo_OffDesign.rho_c_1            = rho_c_1
        Thermo_OffDesign.rho_c_2            = rho_c_2
        Thermo_OffDesign.rho_c_m            = rho_c_m
        Thermo_OffDesign.T_c_m              = T_c_m
        Thermo_OffDesign.rho_h              = rho_h
        Thermo_OffDesign.mu_h               = mu_h
        Thermo_OffDesign.T_h_m              = T_h_m

        return Thermo_OffDesign

    def Pressure_OffDesign(self, sigma_h=None, sigma_c=None, Re_h=None, Re_c=None, eta_o_h=None, eta_o_c=None,
        ...
        """step3: Iteration to match design pressure ratios
        ****************************************************************************"""
        Pressure_OffDesign = HEX_data_OffDesign()

        """actual pressure drop calculation"""
        # determine the entrance and exit coefficients
        K = Kc_Ke_Lib()
        # hot fluid
        Kc_h                            = K.Kc_rect(sigma_h, Re_h)
        Ke_h                            = K.Ke_rect(sigma_h, Re_h)
        # cold fluid
        Kc_c                            = K.Kc_rect(sigma_c, Re_c)
        Ke_c                            = K.Ke_rect(sigma_c, Re_c)
        ...
        """total output results"""
        Pressure_OffDesign.delta_p_h                       = delta_p_h
        Pressure_OffDesign.delta_p_c                       = delta_p_c

        return Pressure_OffDesign

    def OffDesign_Results(self, rho_HEX=2780, eff_FAN=0.7, eff_PUMP=0.7, rho_h=None, N_p=None, H_stack=None,
        ...
        ****************************************************************************"""
        OffDesign_Results = HEX_data_OffDesign()
        ...
        """total output results"""
        OffDesign_Results.A_base_c                        = A_base_c
        OffDesign_Results.A_base_h                        = A_base_h
        OffDesign_Results.A_fr_c                          = A_fr_c
        OffDesign_Results.A_fr_h                          = A_fr_h
        OffDesign_Results.m_HEX                           = m_HEX
        OffDesign_Results.P_HEX_c                         = P_HEX_c
        OffDesign_Results.P_HEX_h                         = P_HEX_h
        OffDesign_Results.P_HEX                           = P_HEX
...
class HEX_data:
    def __init__(self):
        ...
        mu_c                            = []
        mu_h                            = []
        # initial guess of cmv
        NTU                             = []
        G_c                             = []
        G_h                             = []
        ntu_h                           = []
        ntu_c                           = []
        # iteration of pressure drop / core mass velocity pair
        L_h                             = []
        H_stack_h                       = []
        delta_p_h_updated               = []
        sigma_h                         = []
        ...
        A_o_c                           = []
        N_p_c                           = []
        # design results of the compact heat exchanger
        A_base_c                        = []
        A_base_h                        = []
        A_fr_c                          = []
        A_fr_h                          = []
