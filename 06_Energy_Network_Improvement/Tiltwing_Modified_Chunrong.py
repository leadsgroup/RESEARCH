...
#
# Created:                M. Clarke
# Modified:    Aug, 2022, D. Verstraete
# Modified:    Oct, 2022, C.R. Zhao
...
# from MARC.Methods.Weights.Buildups.eVTOL.empty_hydra                           import empty_hydra as empty  # by Verstraete
...
from MARC.Methods.Noise.Fidelity_Zero.Noise_Tools.generate_microphone_points    import generate_building_microphone_points
from MARC.Methods.Geometry.Two_Dimensional.Planform.wing_planform              import wing_planform
from MARC.Methods.Weights.Buildups.eVTOL.converge_evtol_weight                 import converge_evtol_weight  # by Verstraete
from MARC.Plots.Geometry                                                       import *
from MARC.Plots.Performance.Mission_Plots_Dries                                import *
...
def main():
    ...
    run_analysis                                = True
    plot_mission                                = True
    control_points                              = 10    # what does it mean?
    N_gm_x                                      = 10    # what does it mean?
    N_gm_y                                      = 5     # what does it mean?
...
def run_full_mission(simulated_days, flights_per_day, aircraft_range, reserve_segment, run_noise_model,
    ...
    min_y                                       = 1E-1
    max_y                                       = 0.25  * Units.nmi  # flight height is 463 meters
    min_x                                       = 1E-1
    max_x                                       = aircraft_range   # flight range is 92.6 km

    ti                                          = time.time()
    vehicle                                     = vehicle_setup()

    # export the vehicle
    write(vehicle, 'Tiltwing_eVTOL')
    ...
    filename                                    = 'Tiltwing_Battery_Conditions_Ref[150_130]'
...
def run_full_noise_mission(simulated_days, flights_per_day, aircraft_range, reserve_segment, run_noise_model,
    ...
    for i in range(len(X_LIM) - 1):
        for j in range(len(Y_LIM) - 1):
            print('Running Quadrant:' + str(Q_idx))
            min_x                               = X_LIM[i]
            max_x                               = X_LIM[i + 1]
            min_y                               = Y_LIM[j]
            max_y                               = Y_LIM[j + 1]
...
# ----------------------------------------------------------------------
#   run_approach_departure_noise_mission
# ----------------------------------------------------------------------
...
def vehicle_setup():
    ...
    # NETWORKS
    # -----------------------------------------------------------------
    net                                         = MARC.Components.Energy.Networks.Battery_Propeller()
    ...
    # Component 1: the ESC (Electrical Speed Controller)
    ...
    if air_cooled:
        ...
        bat                                         = MARC.Components.Energy.Storages.Batteries.Constant_Mass.Lithium_Ion_LiNiMnCoO2_18650_liquid()
    ...
    bat.module_config.number_of_modules         = 10  # CHANGE IN OPTIMIZER
    # todo either air cooled or liquid cooled, where the temperature of battery changes
    bat.air_cooled                              = air_cooled
    if bat.air_cooled:
            ...
            bat.module_config.normal_count      = 130
            bat.module_config.parallel_count    = 15
    net.battery                                 = bat

    # Component 6: the BTMS
    if not bat.air_cooled:
        btms                                                = MARC.Components.Energy.Cooling.WavyChan_GasLiqHEX()
        btms.hex.length_of_hot_fluid                        = 0.6
        btms.hex.length_of_cold_fluid                       = 0.9
        btms.hex.stack_height                               = 0.24
        btms.hex.hydraulic_diameter_of_hot_fluid_channel    = 5e-4
        btms.hex.aspect_ratio_of_hot_fluid_channel          = 1
        btms.hex.hydraulic_diameter_of_cold_fluid_channel   = 1e-2
        btms.hex.aspect_ratio_of_cold_fluid_channel         = 3

        btms.wavychan.channel_numbers_of_module             = 15

        btms.mass_flow_rate_of_hot_fluid                    = 1  # CHANGE IN OPTIMIZER
        btms.hex.mass_flow_rate_of_cold_fluid               = 2
        btms.hex.pressure_ratio_of_hot_fluid                = 0.7
        btms.hex.pressure_ratio_of_cold_fluid               = 0.98  # CHANGE IN OPTIMIZER
        btms.hex.pressure_at_inlet_of_hot_fluid             = 101325
        btms.energy_calc(bat, total_cells)

        net.btms                                            = btms

    # Component 7: the Rotor
    # Design Rotors
    # -----------------------------------------------------------------
    ...
    prop_rotor                                  = MARC.Components.Energy.Converters.Lift_Rotor()
    ...
    prop_rotor.airfoil_geometry                 = ['./MARC/regression/scripts/Vehicles/Airfoils/NACA_4412.txt']
    prop_rotor.airfoil_polars                   = [['./MARC/regression/scripts/Vehicles/Airfoils/Polars/NACA_4412_polar_Re_50000.txt',
                                                    './MARC/regression/scripts/Vehicles/Airfoils/Polars/NACA_4412_polar_Re_100000.txt',
                                                    './MARC/regression/scripts/Vehicles/Airfoils/Polars/NACA_4412_polar_Re_200000.txt',
                                                    './MARC/regression/scripts/Vehicles/Airfoils/Polars/NACA_4412_polar_Re_500000.txt',
                                                    './MARC/regression/scripts/Vehicles/Airfoils/Polars/NACA_4412_polar_Re_1000000.txt']]
    ...
    # append motor origin span-wise locations onto wing data structure
...
# ----------------------------------------------------------------------
#   Define the Configurations  wing angle changes in geometry
# ---------------------------------------------------------------------
def configs_setup(vehicle):
    """
    The configuration set up below the scheduling of the nacelle angle and vehicle speed.
    Since one propeller operates at varying flight conditions, one must prescribe  the
    pitch command of the propeller which is used in the variable pitch model in the analyses
    ...
    if hover_noise_test:
        ...
        for day in range(simulated_days):
            ...
            for f_idx in range(flights_per_day):
                flight_no = f_idx + 1
                # ------------------------------------------------------------------
                #   First Climb Segment: Constant Speed, Constant Rate [1]
                # ------------------------------------------------------------------ 
                segment                         = Segments.Hover.Climb(base_segment)
                ...
                # ------------------------------------------------------------------
                #   First Cruise Segment: Constant Acceleration, Constant Altitude [4]
                # ------------------------------------------------------------------ 
                ...
                segment.state.unknowns.throttle   = 0.8    * ones_row(1)  # todo check: changed from 0.8 to 0.6
                segment = vehicle.networks.battery_propeller.add_unknowns_and_residuals_to_segment(segment, initial_power_coefficient=0.06)
                mission.append_segment(segment)

                # ------------------------------------------------------------------
                #   First Cruise Segment: Constant Acceleration, Constant Altitude [6]
                # ------------------------------------------------------------------ 
                segment                           = Segments.Climb.Linear_Speed_Constant_Rate(base_segment)
                ...
                # ------------------------------------------------------------------
                #   First Cruise Segment: Constant Acceleration, Constant Altitude [7]
                # ------------------------------------------------------------------ 
                ...
                segment.state.unknowns.throttle  = 0.82    * ones_row(1)   # todo check: changed from 0.8 to  0.82
                segment = vehicle.networks.battery_propeller.add_unknowns_and_residuals_to_segment(segment, initial_power_coefficient=0.01)
                mission.append_segment(segment)

                # ------------------------------------------------------------------
                #    Descent Segment: Constant Acceleration, Constant Altitude [8]
                # ------------------------------------------------------------------ 
                segment                          = Segments.Climb.Linear_Speed_Constant_Rate(base_segment)
                ...
                segment.state.unknowns.throttle  = 0.6      * ones_row(1)  # todo check: changed from 0.6 to  0.65
                segment.battery_energy           = vehicle.networks.battery_propeller.battery.max_energy
                segment = vehicle.networks.battery_propeller.add_unknowns_and_residuals_to_segment(segment, initial_power_coefficient=0.01)  # todo check: changed from 0.01 to  0.03
                mission.append_segment(segment)

                # ------------------------------------------------------------------
                #   Descent Segment: Constant Speed, Constant Rate [11]
                # ------------------------------------------------------------------ 
...
def plot_results(results, run_noise_model, line_style='bo-'):
    ...
    # if False:
    #     # Plot Wavy-channel Performance
    #     plot_wavychannel_conditions(results)
    #     #
    #     # Plot HEX Performance
    #     plot_hex_operating_conditions()
    #     plot_hex_non_operating_conditions()
