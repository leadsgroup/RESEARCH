...
def main():
    ...
    new_simulation      =  True 
    filename            =  'overhead_noise_results'
    use_topograpgy_file =  False 
    
    if new_simulation:
        vehicle  = vehicle_setup()           
        configs  = configs_setup(vehicle) 
        analyses = analyses_setup(configs,use_topograpgy_file)  
...
# ---------------------------------------------------------------------- 
def analyses_setup(configs,use_topograpgy_file):

    analyses = RCAIDE.Framework.Analyses.Analysis.Container()

    # build a base analysis for each config
    for tag,config in configs.items():
        analysis      = base_analysis(config,use_topograpgy_file) 
        analyses[tag] = analysis

    return analyses  


def base_analysis(vehicle, use_topograpgy_file):
...
def plot_results(results,noise_data,regression_plotting_flag): 
    ...
    plot_3D_noise_contour(noise_data,
                        noise_level      = noise_data.L_eq_24hr,
                        min_noise_level  = 20,  
                        max_noise_level  = 90, 
