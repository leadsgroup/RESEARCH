...
def main():
    ...
    if high_fidelity_kin_mech:
        with open('inputs_1.json', 'r') as file:
            dict_fuel = json.load(file)                                             # [-]       Fuel species and corresponding mole fractions for full fuel model
            list_sp   = ['CO', 'CO2', 'H2O', 'NO', 'NO2', 'CSOLID']                 # [-]       Fuel species for Emission Index analysis
    else:
        with open('inputs_2.json', 'r') as file:
            dict_fuel = json.load(file)                                             # [-]       Fuel species and corresponding mole fractions for surrogate fuel model
            list_sp             = ['CO2', 'CO', 'H2O']                              # [-]       Fuel species for Emission Index analysis
    
    with open('Air_input.json', 'r') as file:
        dict_oxy = json.load(file)                                                  # [-]       Air species and corresponding mole fractions    
...
class PSR(object):
    ...
    def __init__(
        ...
        self.t_res = self.rho * self.volume / self.mdot
...
def combustor(
    ...
    if high_fidelity_kin_mech:
        ...
        fuel_file              = ct.Solution('Jet_A_Low_Fidelity.yaml')                # [-]       Import surrogate fuel kinematic mechanism
    fuel_file.TP               = T_stag_0, P_stag_0                                    # [-]       Set the fuel temperature and pressure
    fuel_file.set_equivalence_ratio(phi_PSR[0], fuel=dict_fuel, oxidizer=dict_oxy)     # [-]       Set the euivalence ratio inside the PSR
    fuel_file.equilibrate('HP')                                                        # [-]       Fix the specific enthalpy and pressure
    rho_1                   = fuel_file.density                                        # [kg/m**3] Fuel density
    m_dot_air_1             = m_dot_air_PSR * f_PZ_1                                # [kg/s]    Air mass flow inside the PSR
    m_dot_fuel_file            = m_dot_fuel_PSR * f_PZ_1                               # [kg/s]    Fuel mass flow inside the PSR
    mass_flow_rate_1        = m_dot_fuel_file + m_dot_air_1                            # [kg/s]    Total mass flow inside the PSR
    upstream_1              = ct.Reservoir(fuel_file)                                  # [-]       Create a resevoir for the Fuel upstream of the PSR
    mixer_12                = ct.IdealGasReactor(fuel_file)                            # [-]       Create the reactor for the downstream mixer
    PSR_1                   = ct.IdealGasReactor(fuel_file)                            # [-]       Create the reactor for the PSR
    ...
    Y_fuel_file                = fuel_file[comp_fuel].Y                                   # [-]       Store mass fractions inside the PSR
    ...
    mixer_1234              = ct.IdealGasReactor(fuel_file)                            # [-]       Create the reactor for the downstream mixer
    ...
    mixer_12345678          = ct.IdealGasReactor(fuel_file)                            # [-]       Create the reactor for the downstream mixer
    ...
    mixer_air_1             = ct.IdealGasReactor(fuel_file)                            # [-]       Create the reactor for the downstream mixer
    ...
    PFR_1                   = ct.IdealGasConstPressureReactor(fuel_file)               # [-]       Create the reactor for the PFR
    ...
    mixer_air_2             = ct.IdealGasReactor(fuel_file)                            # [-]       Create the reactor for the downstream mixer
    outlet_PFR_1            = ct.MassFlowController(PFR_1, mixer_air_2, mdot=m_dot_air_1) # [-]       Connect the PFR with the downstream mixer
    t_res_PFR_1             = (fuel_file.density * PFR_1.volume) / (m_dot_air_1)       # [s]       Compute the residence time inside the PFR
    fuel_file.set_equivalence_ratio(phi_SZ_des_1, fuel=dict_fuel, oxidizer=dict_oxy)   # [-]       Set the euivalence ratio inside the PFR
    ...
    PFR_2                   = ct.IdealGasConstPressureReactor(fuel_file)               # [-]       Create the reactor for the PFR
    ...
    mixer_air_3             = ct.IdealGasReactor(fuel_file)                            # [-]       Create the reactor for the downstream mixer
    outlet_PFR_2            = ct.MassFlowController(PFR_2, mixer_air_3, mdot=m_dot_air_2) # [-]       Connect the PFR with the downstream mixer
    t_res_PFR_2             = (fuel_file.density * PFR_2.volume) / (m_dot_air_2)       # [s]       Compute the residence time inside the PFR
    fuel_file.set_equivalence_ratio(phi_SZ_des_2, fuel=dict_fuel, oxidizer=dict_oxy)   # [-]       Set the euivalence ratio inside the PFR
    ...
    PFR_3                   = ct.IdealGasConstPressureReactor(fuel_file)               # [-]       Create the reactor for the PFR
    ...
    t_res_PFR_3             = (fuel_file.density * PFR_3.volume) / (m_dot_air_3)       # [s]       Compute the residence time inside the PFR
    fuel_file.set_equivalence_ratio(phi_SZ_des_3, fuel=dict_fuel, oxidizer=dict_oxy)   # [-]       Set the euivalence ratio inside the PFR
    ...
    Emission_Index          = fuel_file.Y * (m_dot_input_combustor)/m_dot_fuel         # [-]       Computation of the Emission Index
    a_out                   = fuel_file.sound_speed                                    # [m/s]     Speed of sound at PFR outlet
    rho_out                 = fuel_file.density                                        # [kg/m**3] Density of the fuel_file
    gamma                   = fuel_file.cp_mass / fuel_file.cv_mass                       # [-]       Heat capacity ratio
    h                       = fuel_file.h                                              # [J/kg]    Enthalpy
    vel_out                 = (m_dot_input_combustor) / (rho_out * A_SZ)            # [m/s]     Outlet velocity
    M_out                   = vel_out / a_out                                       # [-]       Outlet Mach number
    T_stag_out              = fuel_file.T * (1 + 0.5 * (gamma - 1) * (M_out)**2)       # [K]       Stagnation temperature
    P_stag_out              = fuel_file.P * (1 + 0.5 * (gamma - 1) * (M_out)**2)**(gamma / (gamma - 1)) # [Pa]      Stagnation pressure
    h_stag_out              = T_stag_out  * fuel_file.cp_mass                          # [J/kg]    Stagnation enthalpy

    return (fuel_file, Emission_Index, T_stag_out, P_stag_out, h_stag_out)
