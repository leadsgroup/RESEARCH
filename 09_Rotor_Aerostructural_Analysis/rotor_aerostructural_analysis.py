...
def lift_rotor_run():     
    ...
    rotor.design_angular_velocity             = rotor.design_tip_mach* 343 /rotor.tip_radius  
    rotor.design_freestream_velocity          = np.sqrt(rotor.design_thrust/(2*1.2*np.pi*(rotor.tip_radius**2))) 
    ...
    conditions.frames.inertial.velocity_vector          = np.array([[0, 0. ,rotor.design_freestream_velocity]])  
    conditions.propulsion.throttle                      = np.array([[1.0]])
    conditions.frames.body.transform_to_inertial        = np.array([[[1., 0., 0.],[0., 1., 0.],[0., 0., -1.]]])      

    # Assign rpm
    rotor.inputs.omega  = np.array(rotor.design_angular_velocity,ndmin=2) 
