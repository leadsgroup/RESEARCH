...
def main():
    ...
    num_segments       =  5  
    propellants = ['Liquid_Natural_Gas']
    ...
    for p_i in  range(len(propellants)):
        ...
        df      = pd.DataFrame({'Time': time, 'emissions_CO2': emissions_CO2, 'emissions_H2O': emissions_H2O, 'emission_index_CO2': emission_index_CO2, 'emission_index_H2O': emission_index_H2O}) 

        df.to_csv(propellants[p_i] + '_EmissionsB737.csv', index=False)       
...
def vehicle_setup(propellant): 
    ...
    turbofan.tag                                = 'starboard_propulsor' # Reg 
    ...
    turbofan.design_thrust                      = 24376.0* Units.N 
