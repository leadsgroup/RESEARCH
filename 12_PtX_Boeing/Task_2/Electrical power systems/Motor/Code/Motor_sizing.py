...
def main():
    ...
    Stack          = 0.2                         # [m]            motor stack length
    L              = 11.69                         # wire length
    N              = 1                         # [-]            number of turns   
    ...
    l              = 0.2                         # [m]            length of the path    
    l_g            = 0.001                       # [m]            airgap size
    Rm             = 0.169                       # [m]            magnet outer radius
    Rr             = 0.101                       # [m]            magnet inner radius
    Rs             = 0.09                        # [m]            stator inner radius
    theta          = 0                           # [rad]          angular position
        
    motor_func_1(D_out, D_in, N, I, l, mu_0, mu_r, k_w, L)
    motor_func_2(D_out, D_in, N, I, l,  mu_0, mu_r, k_w , L)
    motor_func_3(D_out, D_in, l_g, mu_0,  l_m,  mu_r, Br, k_w,  Stack,  I)
    motor_func_4(Br,  p,  Rr,  Rm,  Rs,  theta,  D_in,  k_w,  I,  Stack)
...
def motor_func_1(D_out, D_in, N, I, l, mu_0, mu_r, k_w, L):
    ...
    B              = 1.2                                                   # [V*s/m**2]     average magnitude of the radial flux density produced by the rotor
    A_sign         = (k_w*I)/(np.pi*D_in)                                  # [-]            stator electrical loading (Eq.2)    
    Q              = (np.pi/2)*(B*A_sign)*(D_in**2)*L                  # [Nm]           torque (Eq.1)
    return B, Q

def motor_func_2(D_out, D_in, N, I, l,  mu_0, mu_r, k_w , L ):
    ...
    B              = phi/A
    A_sign         = (k_w*I)/(np.pi*D_in)                                  # [Wb/m**2]      magnetic flux density (Eq.13)     
    Q              = (np.pi/2)*(B*A_sign)*(D_in**2)*L                  # [Nm]           torque (Eq.1)      
    
    return B, Q

def motor_func_3(D_out, D_in, l_g, mu_0,  l_m,  mu_r, Br, k_w,  Stack,  I):
       
    A_m            = np.pi * ((D_out**2 - D_in**2) / 4)                    # [m**2]         magnet area      
    ...
    B              = (Br*l_m)/(l_g*mu_r + l_m)                             # [Wb/m**2]      airgap field in the gap produced by the magnets (Eq.20)
    A_sign         = (k_w*I)/(np.pi*D_in)                                  # [-]            stator electrical loading (Eq.2)    
    Q              = (np.pi/2)*(B*A_sign)*(D_in**2)*Stack                  # [Nm]           torque (Eq.1)
    
    return B, Q

def motor_func_4(Br,  p,  Rr,  Rm,  Rs,  theta,  D_in,  k_w,  I,  Stack):
     
    B_r_value = ((Br * (p / (p + 1)) * (1 - (Rr / Rm)**(p + 1))) / (1 - ((Rr**(2*p))/(Rs)))) * (((Rs / Rs)**(p - 1)) * ((Rm / Rs)**(p + 1)) + ((Rm / Rs)**(p + 1))) * np.cos(p * theta)
    
    B_sign         = (2/np.pi)*B_r_value                                   # average airgap field (Eq.25)    
    A_sign         = (k_w*I)/(np.pi*D_in)                                  # [Wb/m**2]      magnetic flux density (Eq.13)     
    Q    = (np.pi/2)*(B_sign*A_sign)*(D_in**2)*Stack             # [Nm]           torque (Eq.1)         
    return B_g1, Q
