...
from SUAVE.Methods.Power.Battery.compute_net_generated_battery_heat_liquid import compute_net_generated_battery_heat_liquid
...
class Lithium_Ion_LiNiMnCoO2_18650_liquid(Lithium_Ion):
    ...
    def energy_calc(self,numerics,atmospheric_conditions, btms, discharge_flag=True):
        ...
        # added at 30/11/2022
        air_cooled               = battery.air_cooled
        # todo added at 12/12/2022
        T_i                      = btms.operating_conditions.wavy_channel_inlet_temperature
        T_o                      = btms.operating_conditions.wavy_channel_outlet_temperature
        T_c_1                    = btms.operating_conditions.heat_exchanger_inlet_temperature_of_cold_fluid
        T_c_2                    = btms.operating_conditions.heat_exchanger_outlet_temperature_of_cold_fluid
        T_h_2                    = btms.operating_conditions.heat_exchanger_outlet_temperature_of_hot_fluid
        ...
        # Li-ion battery internal resistance
        R_0      =  0.01483*(SOC_old**2) - 0.02518*SOC_old + 0.1036

        # Voltage under load:
        V_oc      = V_ul + (I_cell * R_0)

        # modified by Chunrong at 28/10/2022
        q_dot_entropy  = -T_cell*delta_S*I_cell/(n*F)
        q_dot_joule    = I_cell * (V_oc - V_ul)
        Q_heat_gen     = (q_dot_joule + q_dot_entropy)
        ...
        if air_cooled:
            ...
            btms            = compute_net_generated_battery_heat_liquid(n_total,battery,atmospheric_conditions,Q_heat_gen,numerics, btms)
            T_current       = btms.operating_conditions.battery_current_temperature
    ...
    def append_battery_unknowns(self,segment): 
        ...
        # todo added at 9/12/2022 by ZHAO
        if not self.air_cooled:
            propulsion.btms.wavy_channel_inlet_temperature[1:,:] = segment.state.unknowns.wavy_channel_inlet_temperature[1:,:]

        # print(segment.tag)
        # print('discharge time:\n', segment.state.conditions.frames.inertial.time.T / Units.min, end='\n')
        # print('state of charge:\n', propulsion.battery_state_of_charge.T, end='\n')
        # print('battery temperature:\n', propulsion.battery_cell_temperature.T, end='\n')
        # print('coolant inlet temperature:\n', propulsion.btms.wavy_channel_inlet_temperature.T, end='\n')
        return     
    ...
    def append_battery_residuals(self,segment,network): 
        ...
        # todo added at 9/12/2022 by ZHAO
        if not self.air_cooled:
            coolant_temp_actual  = segment.state.conditions.propulsion.btms.wavy_channel_inlet_temperature
            coolant_temp_predict = segment.state.unknowns.wavy_channel_inlet_temperature
            segment.state.residuals.network.coolant_temp = coolant_temp_predict - coolant_temp_actual
    ...
    def append_battery_unknowns_and_residuals_to_segment(self,segment,initial_voltage,
        ...
        # todo added at 9/12/2022 by Zhao
        if not self.air_cooled:
            wavy_channel_inlet_temperature = 284.  # set one degree below the cell temperature todo change to 284 from 283
            segment.state.unknowns.wavy_channel_inlet_temperature = wavy_channel_inlet_temperature * ones_row(1)
